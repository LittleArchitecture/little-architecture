 参数传递-JSON
 	  访问路径：http://localhost:8080/LittleArchitecture/system/UserController/get
 	  
文件上传
	 commons-io-1.3.2.jar
	 commons-fileupload-1.2.1.jar
 	  访问路径： http://localhost:8080/LittleArchitecture/system/UploadfileController/  （ 分普通上传和 CommonsMultipartResolver两种上传方式）
...
Spring与Spring MVC集成
	访问路径： http://localhost:8080/LittleArchitecture/system/UserController/get2
	步骤：
		1、web.xml 配置spring；
		2、编写实现接口和接口实现类；
		3、Controller实现；
...
Spring与Hibernate集成
	访问路径： http://localhost:8080/LittleArchitecture/system/UserController/getAllUser
	
Spring与Hibernate集成  基于REST风格实现用户管理
	访问路径： http://localhost:8080/LittleArchitecture/system/UserControllerRest/get
	
Spring与Hibernate集成  基于REST风格实现JSON用户显示
	访问路径： http://localhost:8080/LittleArchitecture/system/UserControllerRest/{id}/{username}
